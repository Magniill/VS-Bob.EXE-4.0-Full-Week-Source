...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		DiscordClient.changePresence(detailsText + " " + cooltext + " (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy), "\nAcc: " + HelperFunctions.truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses  , iconRPC);
		#end
		...
		switch(stageCheck)
		{
			...
			case 'sunkStage':
			{
				...
				var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('lolBG', 'exe'));
			...
			case 'mag-staging':
			{
				...
				var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('magStageThing', 'exe'));
				bg.setGraphicSize(Std.int(bg.width * 0.8));
				bg.antialiasing = true;
				bg.scrollFactor.set(.91, .91);
				bg.x -= 670;
		...
		if (!PlayStateChangeables.Optimize)
		{
			if (curStage != 'sonicFUNSTAGE' && curStage != 'LordXStage'  && curStage != 'sunkStage') add(gf);
		...
		if (isStoryMode)
		{
			switch (StringTools.replace(curSong," ", "-").toLowerCase())
			{
				...
				else if (curSong.toLowerCase() == 'little-man')
				{
					add(blackFuck);
					startCircle.loadGraphic(Paths.image('StartScreens/CircleLittleMan', 'exe'));
					startCircle.x += 777;
					add(startCircle);
					startText.loadGraphic(Paths.image('StartScreens/TextLittleMan', 'exe'));
					startText.x -= 1200;
					add(startText);
				...
				else if (curSong.toLowerCase() == 'sussus-toogus')
				{
					add(blackFuck);
					startCircle.loadGraphic(Paths.image('StartScreens/CircleMagniill', 'exe'));
					startCircle.x += 777;
					add(startCircle);
					startText.loadGraphic(Paths.image('StartScreens/TextMagniill', 'exe'));
					startText.x -= 1200;
					add(startText);
	...
	function doJumpscare(sound:Int = 0, opa:Int = 0)
		{
			...
			{
				case 0:
				FlxG.sound.play(Paths.sound('jumpscare', 'exe'), 0);
				FlxG.sound.play(Paths.sound('datOneSound', 'exe'), 0);
				case 1:
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			#if windows
			DiscordClient.changePresence("PAUSED on " + cooltext + " (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy), "Acc: " + HelperFunctions.truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses  , iconRPC);
			#end
	...
	override public function update(elapsed:Float)
	{
		...
		if (health <= 0)
		{
			...
			DiscordClient.changePresence("GAME OVER -- " + cooltext + " (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy),"\nAcc: " + HelperFunctions.truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses  , iconRPC);
			#end
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					else if (curSong.toLowerCase() == 'too-slow' && storyDifficulty == 2 && !FlxG.save.data.soundTestUnlocked && !PlayStateChangeables.botPlay)
					{
						var video:MP4Handler = new MP4Handler();
					...
					else if (curSong.toLowerCase() == 'too-slow' && storyDifficulty != 2 && !FlxG.save.data.soundTestUnlocked && PlayStateChangeables.botPlay) LoadingState.loadAndSwitchState(new UnlockScreen(false, 'soundtest'));
	...
	*/
	...
	override function stepHit()
	{
		...
		if (curSong == 'milk')
		{
			...
			{
				var sponge:FlxSprite = new FlxSprite(dad.getGraphicMidpoint().x - 200, dad.getGraphicMidpoint().y - 120).loadGraphic(Paths.image('SpingeBinge', 'exe'));
		...
		// Updating Discord Rich Presence (with Time Left)
		DiscordClient.changePresence(detailsText + " " + cooltext + " (" + storyDifficultyText + ") " + Ratings.GenerateLetterRank(accuracy), "Acc: " + HelperFunctions.truncateFloat(accuracy, 2) + "% | Score: " + songScore + " | Misses: " + misses  , iconRPC,true,  songLength - Conductor.songPosition);
		#end
